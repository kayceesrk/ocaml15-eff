...
let rec echo_server tid sock addr =
  try
    printf "echo_server: Thread %d - waiting for data..\n%!" tid;
    let data = recv sock 1024 in
    printf "echo_server: Thread %d - got data!\n%!" tid;
    if String.length data > 0 then
      (ignore (send sock data);
       echo_server tid sock addr)
    else
...
let server () =
  ...
  try
    ...
    while true do
      ...
      let tid = !tid_ref in
      incr tid_ref;
      Aio.fork (fun () -> echo_server tid client_sock client_addr)
    done
  with
  | _ -> close ssock
