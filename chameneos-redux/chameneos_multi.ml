...
let fork f =
  let tgt = !fork_idx in
  let fork_idx = (tgt + 1) mod num_domains in
  Sched.fork_on f tgt
...
let main () =
  ...
  work [ C.Blue; C.Red; C.Yellow ] num_domains;
  printf "\n";
  work [ C.Blue; C.Red; C.Yellow; C.Red; C.Yellow; C.Blue; C.Red; C.Yellow; C.Red; C.Blue ] num_domains;
  printf "\n"
